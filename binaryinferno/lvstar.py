...
def inferlvstar(txt):
    ...
    for i in range(max_k):
        ...
        if notallleadingzeros(ys) and all([canapplylv(y) for y in ys]):
            # print([len(y)-len(applylv(y)) for y in ys])
            intervals = [INTERVAL("V",i,i+len(y)-len(applylv(y))) for y in ys]
            sigmas.append(SIGMA([FIELD(intervals,annotation="Repeated Length Value Pairs")]))
