...
def dumpsol(sol,msgs,OFFSET=None,PUSH=0):
    print("Will push",PUSH)
    ...
    for raw_field in raw_fields:
        ...
        for interval in raw_field:
            if PUSH > 0:
                print("Intervaling PUSHed",INTERVAL("R",interval[1],interval[2]),"pushed to ",INTERVAL("R",PUSH+interval[1],PUSH+interval[2]))
                intervals.append(INTERVAL("R",PUSH+interval[1],PUSH+interval[2]))
            else:
                intervals.append(INTERVAL("R",interval[1],interval[2]))
        annotation = raw_field[0][0]
        field = FIELD(intervals,annotation=annotation,valuescale=1)
        fields.append(field)
    s = SIGMA(fields)
    print("Final Sigma",s)
...
def infer_reps(txtmsgs,offset=0,txtfile=None,qtysols=1,mhl=300,answerfmt=False,shortcircuit=None,filterrules=None,push=0):
    ...
    def dumpf(sol,OFFSET=None,PUSH=push):
        return dumpsol(sol,msgs,OFFSET,PUSH)
