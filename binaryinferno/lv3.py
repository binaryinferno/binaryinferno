...
def inferlength3(txt,endian=">"):
  ...
  def slice(xs,i,k):
    if endian == ">":
      return [struct.unpack(endian+"I",bytes([0])+x[i:i+k])[0] for x in xs]
    else:
      return [struct.unpack(endian+"I",x[i:i+k]+bytes([0]))[0] for x in xs]
  ...
  for j,i in enumerate(valid):
    ...
    #return s + "\n".join(lines)+"\n--"
    intervals = [INTERVAL("L",i,i+3) for k in range(len(xs))]
    ...
    if endian == ">":
      ...
      end_anno ="LE"
    sig = SIGMA([FIELD(intervals,annotation=end_anno+" uint24 Length +" + " " + str(diff_vals[j][0]) + " = Total Message Length",valuescale=WCAT1)])
    #print(sig)
    #print(sig.apply(txt))
    sigs.append(sig)
  for s in sigs:
...
def inferlength3BE(txt):
  return inferlength3(txt,endian=">")

def inferlength3LE(txt):
  return inferlength3(txt,endian="<")
