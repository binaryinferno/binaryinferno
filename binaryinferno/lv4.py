...
def inferlength4(txt,endian=">"):
  ...
  def slice(xs,i,k):
    return [struct.unpack(endian+"I",x[i:i+k])[0] for x in xs]
  ...
  for j,i in enumerate(valid):
    ...
    #return s + "\n".join(lines)+"\n--"
    intervals = [INTERVAL("L",i,i+4) for k in range(len(xs))]
    ...
    if endian == ">":
      ...
      end_anno ="LE"
    sig = SIGMA([FIELD(intervals,annotation=end_anno+" uint32 Length +" + " " + str(diff_vals[j][0]) + " = Total Message Length",valuescale=WCAT1)])
    #print(sig)
    #print(sig.apply(txt))
    sigs.append(sig)
  for s in sigs:
...
def inferlength4BE(txt):
  return inferlength4(txt,endian=">")

def inferlength4LE(txt):
  return inferlength4(txt,endian="<")
