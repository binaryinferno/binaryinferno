...
def sumeng(msgs=None,goal=None,width=None):
    def H(xs_):
...
def prettyprintsol(sol,i=""):
    match_gt,params = sol #(None, (1.585, 0, -1, 1, <built-in function sub>, <built-in function add>, '0xc9c4', 0.96875))
    entropy , msg_start, msg_end,  candidate_index, fold_op, final_op, magic_va, payload_ratio = params
    if match_gt == None:
        match_gt_s = ""
    else:
        if match_gt:
...
if __name__ == "__main__":
    ...
    msgs = """D00771BCBE3B0000
E80317FC00000000
D00771BCBE3B0000
4C04EFBFBE3B0600
0B0007E1BE3B01007E457874656E64466D74"""
    ...
    #     width = int(sys.argv[1])
    #     fname = sys.argv[2]
    from generate_code import code_soln
    res = sumeng(msgs=msgs,goal=None,width=width)
    ...
    if res != []:
        ...
        if min_sol <= desired_sol and desired_sol <= max_sol:
            s = res[desired_sol]
            #print("Making Sol ", desired_sol,s)
            code_soln(s,msgs)




    # res = sumeng(msgs=msgs)
