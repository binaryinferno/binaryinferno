...
# sketch
    # apply lvs until you can't, undo the can't 


def notallleadingzeros(xs):
    return not all([x[0] ==0 for x in xs])

def notallsame(xs):
    return not len(set([x[0]  for x in xs]))==1
...
def inferlvone(txt):
    ...
    for i in range(max_k):
        ...
        #print(i,notallleadingzeros(ys),[canapplylv(y) for y in ys])
        if notallsame(ys) and notallleadingzeros(ys) and all([canapplylv(y) for y in ys]):
            # print([len(y)-len(applylv(y)) for y in ys])
            intervals = [INTERVAL("O",i,i+len(y)-len(applylv(y))) for y in ys]
            sigmas.append(SIGMA([FIELD(intervals,annotation="One Length Value Pair",valuescale=.2)]))
        ...
        #     s = SIGMA([FIELD(intervals,annotation="Float ",valuescale=.1)])
        #     sigmas.append(s)
