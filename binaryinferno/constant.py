...
# If you wanted a detector which could infer that something is a "constant valued field"
...
def inferconstant(txt):
    ...
    for i in range(max_k):
        ys = [x[i] for x in xs]
        allsame = len(set(ys))==1
        #print(i,all(are_ips),ys)
        if allsame:
            intervals = [INTERVAL("C",i,i+1) for x in xs]
            s = SIGMA([FIELD(intervals,annotation="Constant",valuescale=.01)])
            sigmas.append(s)
